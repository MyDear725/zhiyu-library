...
知遇不把图书馆理解为一组分散页面，而是将“人 × 知识 × 空间”编排为一个可行动的系统：

- **Action AI Librarian**：用户表达真实学习目标后，系统输出知识、空间和同伴三段式行动计划。书名、馆藏位置、可借状态、楼层和人数来自本地馆藏与数据库；未配置 API Key 时使用本地规则回退。
- **Personal Knowledge Twin**：仅本人可见的知识地图，基于借阅清单与安全行为事件生长；没有行为时明确展示空状态。
- **Unified Activity Events**：搜索、加入借阅、选座、学习意图、社区发言、馆内问答、计划生成和签退写入统一事件层。事件写入失败不会阻断主流程。
- **Library Intelligence**：演示运营视图只展示匿名聚合 KPI 与主题趋势，不返回姓名、学号或原始私密内容。
- **可扩展 Engine**：Space Engine 已定义学习目的与可用座位的推荐契约；People Engine 已定义匿名主题与聚合人数契约，后续可接入学习气候地图和匿名问题墙。

### 隐私与演示数据

`activity_events` 不记录密码、会话 token、学号和原始问题/消息文本。低曝光馆藏推荐是基于演示馆藏的探索权重，界面会明确标识，不会伪造成真实历史借阅统计。运营洞察为“演示运营视图”，所有数据均为匿名聚合。

### 新增演示流程

1. 登录后在首页进入“AI馆员”，输入一个真实学习目标。
2. 查看可跳转至借书、选座、同伴广场的行动卡片。
3. 返回“我的”查看只属于自己的 Knowledge Twin。
4. 通过页脚进入“运营洞察”，展示图书馆获得的匿名服务价值。
